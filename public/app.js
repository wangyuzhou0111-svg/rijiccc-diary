...
const documentTypes = [
  ...
  { value: "blog", label: "博客", title: "新的博客" },
  { value: "letter", label: "写信", title: "新的信" },
];
...
const blogSummaryInput = $("#blogSummaryInput");
...
function getNewEntryKind() {
  return libraryFilter === "all" ? "diary" : libraryFilter;
}
...
function createEntry(shouldRender = true, kind = getNewEntryKind()) {
  const now = new Date().toISOString();
  const type = getDocumentType(kind);
  const entry = {
    id: makeId(),
    kind: type.value,
    title: type.title,
    ...
    images: [],
    versions: [],
    blog: { slug: "", summary: "", status: "draft" },
    letter: { phone: "", greeting: "" },
    createdAt: now,
    updatedAt: now
  };
...
function collectActiveEntry() {
  ...
  entry.kind = docTypeSelect.value || "diary";
  ...
  entry.blog = {
    ...
    status: blogStatusSelect.value || "draft",
  };
  entry.letter = {
    phone: letterPhoneInput.value.trim(),
    greeting: letterGreetingInput.value.trim(),
  };
...
function renderEditor() {
  ...
  blogSlugInput.value = entry.blog.slug || "";
  blogSummaryInput.value = entry.blog.summary || "";
  blogStatusSelect.value = entry.blog.status || "draft";
  letterPhoneInput.value = entry.letter.phone || "";
  letterGreetingInput.value = entry.letter.greeting || "";
  renderContextFields();
  updateCounts();
  renderImages();
  renderVersions();
...
function renderContextFields() {
  ...
  blogFields.hidden = kind !== "blog";
  letterFields.hidden = kind !== "letter";
}
function renderEntryList() {
  const query = searchInput.value.trim().toLowerCase();
  const visible = entries.filter((entry) => {
    entry.kind = entry.kind || "diary";
    const typeMatch = libraryFilter === "all"
      || (libraryFilter === "favorite" ? entry.favorite : false)
      || (libraryFilter === "pinned" ? entry.pinned : false)
      || entry.kind === libraryFilter;
  ...
  entryList.innerHTML = visible.map((entry) => {
    ...
    const pinnedClass = entry.pinned ? " is-pinned" : "";
    ...
    const pinClass = entry.pinned ? " is-pinned" : "";
    const pinLabel = entry.pinned ? "取消置顶" : "置顶这篇";
    return `<article class="entry-card${activeClass}${pinnedClass}" data-id="${entry.id}"><button class="entry-open-button" data-open-entry="${entry.id}" type="button"><span class="directory-line"><strong>${escapeHtml(entry.title || "没有标题")}</strong><em class="saved-mark">${entry.pinned ? "置顶" : escapeHtml(type.label)}</em></span><span>${formatDateLabel(entry.date)} · ${escapeHtml(entry.mood || "心情")} · ${escapeHtml(entry.weather || "天气")}</span><span>${escapeHtml(text)}</span></button><button class="entry-pin-button${pinClass}" data-toggle-pin="${entry.id}" type="button" aria-label="${pinLabel}">↑</button><button class="entry-favorite-button${favoriteClass}" data-toggle-favorite="${entry.id}" type="button" aria-label="${favoriteLabel}">★</button><button class="entry-delete-button" data-delete-entry="${entry.id}" type="button" aria-label="删除这篇作品">删除</button></article>`;
  }).join("");
}
function compareEntries(a, b) {
  if (Boolean(a.pinned) !== Boolean(b.pinned)) return a.pinned ? -1 : 1;
  const sortMode = sortSelect?.value || "updated-desc";
  ...
  if (sortMode === "title-asc") return String(a.title || "").localeCompare(String(b.title || ""), "zh-Hans-CN");
  if (sortMode === "words-desc") return countWords(plainTextFromHtml(b.body || "")) - countWords(plainTextFromHtml(a.body || ""));
...
function togglePin(entryId) {
  ...
  saveEntries();
  renderEntryList();
  setStatus(entry.pinned ? "已经置顶这篇作品。" : "已经取消置顶。");
}
...
function formatTimer(seconds) {
  ...
  return hours
    ? `${String(hours).padStart(2, "0")}:${String(shownMinutes).padStart(2, "0")}:${String(rest).padStart(2, "0")}`
    : `${String(minutes).padStart(2, "0")}:${String(rest).padStart(2, "0")}`;
}
function renderTimer() {
  timerDisplay.textContent = formatTimer(timerSeconds);
  timerBadge.textContent = timerId ? "计时中" : (timerSeconds ? "已暂停" : "未开始");
}
...
function loadWritingTimer() {
  timerSeconds = Math.max(0, Number(localStorage.getItem(TIMER_KEY) || 0));
  renderTimer();
}
...
function runDiaryAiTools(tools) {
  ...
  (tools || []).forEach((tool) => {
    ...
    if (name === "append_diary") {
      ...
      used.push("设置标签");
    } else if (name === "set_type" && documentTypes.some((type) => type.value === String(value))) {
      docTypeSelect.value = String(value);
      renderContextFields();
      used.push("设置类型");
    } else if (name === "set_blog_summary") {
      blogSummaryInput.value = String(value || "").trim();
      used.push("设置博客摘要");
    } else if (name === "set_blog_slug") {
      blogSlugInput.value = makeSlug(value);
      used.push("设置博客地址名");
    } else if (name === "set_letter_phone") {
      letterPhoneInput.value = String(value || "").trim();
      used.push("设置手机号");
    } else if (name === "set_letter_greeting") {
      letterGreetingInput.value = String(value || "").trim();
      used.push("设置信件称呼");
    }
  });
  collectActiveEntry();
...
function exportMarkdown() {
  ...
  lines.push("");
  lines.push(`类型：${getDocumentType(entry.kind || "diary").label}`);
...
function exportHtml() {
  collectActiveEntry();
  const entry = getActiveEntry();
  const summary = entry.blog?.summary || plainTextFromHtml(entry.body).slice(0, 120);
...
<body>
  <article>
    ...
    <p>${escapeHtml(formatDateLabel(entry.date))} · ${escapeHtml(getDocumentType(entry.kind || "diary").label)}</p>
    ${entry.body || ""}
  </article>
</body>
</html>`;
  const filename = `${entry.blog?.slug || makeSlug(entry.title)}.html`;
  exportFile(filename, html, "text/html;charset=utf-8");
}
function exportJson() {
  collectActiveEntry();
  const entry = getActiveEntry();
...
function exportAllJson() {
  ...
  exportFile(`rijiccc-all-${todayString()}.json`, JSON.stringify(payload, null, 2), "application/json;charset=utf-8");
}
...
function buildLetterText(entry) {
  const greeting = entry.letter?.greeting?.trim();
  const body = plainTextFromHtml(entry.body).trim();
  return [greeting, body].filter(Boolean).join("\n\n");
}
function sendLetterBySms() {
  collectActiveEntry();
  const entry = getActiveEntry();
  const phone = (entry.letter?.phone || "").replace(/[^\d+]/g, "");
  const message = buildLetterText(entry);
  if (entry.kind !== "letter") {
    setStatus("请先把类型改成“写信”。");
    return;
  }
  if (!phone) {
    setStatus("请先填写收信人的手机号。");
    return;
  }
  if (!message) {
    setStatus("信的正文还是空的，请先写一点内容。");
    return;
  }
  const ok = window.confirm(`要打开“信息”给 ${phone} 发送这封信吗？打开后还需要你自己点发送。`);
  if (!ok) return;
  ...
  setStatus("已经打开短信发送界面，请你检查后再点发送。");
}
function setupVoice() {
...
function bindEvents() {
  ...
  $("#timerStartBtn").addEventListener("click", startWritingTimer);
  ...
  entryList.addEventListener("click", (event) => {
    ...
    if (pinButton) {
      togglePin(pinButton.dataset.togglePin);
      return;
    }
...
function boot() {
  fillSelect(docTypeSelect, documentTypes.map((type) => type.label));
  docTypeSelect.innerHTML = documentTypes.map((type) => `<option value="${type.value}">${escapeHtml(type.label)}</option>`).join("");
